...
class Main:
    ...
    def _update_title(self):
        while True:
            os.system(f'title [Discord Reporter] - hits: {self.sent} ^| fails: {self.errors}')
            time.sleep(0.1)

    def _multi_threading(self):
        threading.Thread(target=self._update_title).start(
        if) True: threading.Thread(target=self._reporter).start()
...
if __name__ == '__main__':
    os.system('cls && title [discort: discord report] - zxlpvx')
    main = Main()
    main.setup()
